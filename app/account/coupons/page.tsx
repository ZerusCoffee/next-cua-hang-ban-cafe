...
export default function CouponsPage() {
  ...
  const renderCouponCard = (
    ...
    return (
      <div
        key={coupon.id}
        className={`coupon-card group relative flex flex-col md:flex-row bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm transition-all duration-500 ${
          status === "active"
            ? "hover:shadow-xl hover:shadow-[#D94E28]/5 hover:border-[#D94E28]/20"
            : "opacity-70 grayscale-[0.3]"
        }`}
      >
        {/* Left Section: Price Stub */}
        <div
          className={`md:w-36 flex flex-col items-center justify-center p-6 text-white shrink-0 relative transition-colors duration-500 ${
            status === "active"
              ? "bg-linear-to-br from-[#D94E28] to-[#FF8C66]"
              : status === "used"
                ? "bg-stone-400"
                : "bg-red-400"
          }`}
        >
          ...
          <div className="text-[9px] font-black uppercase tracking-widest opacity-90 text-center leading-none">
            {isPercentage ? "Giảm giá" : "Giảm trực tiếp"}
          </div>

          {/* Half circles cut-out effect */}
          <div className="absolute -right-3 top-0 bottom-0 w-6 flex flex-col justify-around py-2 z-10 hidden md:flex">
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
          ...
          <div className="mt-4 pt-4 border-t border-stone-50 flex flex-wrap items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-stone-700 font-bold">
                Đơn từ {coupon.minimum_order_amount.toLocaleString()}đ
              </p>
            </div>
          ...
          <div className="lg:col-span-3">
            <Tabs defaultValue="active" className="w-full">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                <TabsList className="bg-white p-1.5 rounded-[1.5rem] border border-stone-100 shadow-sm inline-flex h-auto">
                  <TabsTrigger
                    value="active"
                    className={`rounded-xl px-8 py-2.5 text-xs font-black uppercase tracking-widest transition-all data-[state=active]:text-white data-[state=active]:bg-[#D94E28]`}
                  >
                    Sẵn sàng
                  </TabsTrigger>
                  <TabsTrigger
              ...
              <TabsContent value="active" className="mt-0 outline-hidden">
                {isLoading ? (
                  ...
                  <div ref={listRef} className="grid grid-cols-1 gap-6">
                    {coupons
                      ...
                      .map((c) => renderCouponCard(c, "active"))}
                  </div>
                ) : (
                  <div className="bg-white rounded-[3rem] p-24 text-center border-2 border-dashed border-stone-100 flex flex-col items-center">
                    ...
                    <p className="text-stone-400 text-sm max-w-sm font-medium mb-10 leading-relaxed">
                      Đừng lo, Zerus luôn có ưu đãi mới mỗi ngày. Khám phá ngay
                      để không bỏ lỡ!
                    </p>
                    <Button
                      variant="outline"
              ...
              <TabsContent value="used" className="mt-0 outline-hidden">
                <div className="py-32 text-center bg-stone-50/50 rounded-[3rem] border-2 border-dashed border-stone-100">
                  <p className="text-stone-400 font-bold uppercase tracking-widest italic">
                    Lịch sử dùng mã sẽ hiển thị ở đây
                  </p>
                </div>
              </TabsContent>

              <TabsContent value="expired" className="mt-0 outline-hidden">
                <div className="py-32 text-center bg-stone-50/50 rounded-[3rem] border-2 border-dashed border-stone-100">
                  <p className="text-stone-400 font-bold uppercase tracking-widest italic">
